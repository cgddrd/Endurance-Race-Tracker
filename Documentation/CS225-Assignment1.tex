...
This will allow other entrant checkpoint time values to be logged while an entrant is waiting at a medical checkpoint, and prevents ``time being re-written" by having to insert arrival times ``in the past" within the times log file. This could however be acheived using a priority queue data structure.
